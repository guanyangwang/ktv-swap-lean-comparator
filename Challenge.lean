...
/-!
Root-level Mathlib-only challenge statement for the fixed-margin lazy
swap-chain spectral-gap theorem.

This file intentionally imports exactly `Mathlib`.  The definitions below spell
out the binary fixed-margin state space, the ordered-distinct rectangle
proposal, the `1/2`-lazy operator, and the rational Poincare spectral-gap
predicate needed to state the theorem.
-/
...
/--
Mathlib-only statement of the fixed-margin lazy swap-chain spectral-gap bound.
The proposal chooses ordered distinct row and column pairs and then applies a
`1/2` lazy step.
-/
